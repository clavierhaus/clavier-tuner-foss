...
"$ADB" get-state >/dev/null 2>&1 || { echo "!! no phone: run w-adb (or plug in the cable)"; exit 1; }
SRC=""
for d in /sdcard/Recordings/ClavierTuner /sdcard/Music/ClavierTuner; do
  if "$ADB" shell "test -d $d" 2>/dev/null; then SRC="$d"; break; fi
done
[ -n "$SRC" ] || { echo "!! nothing recorded yet"; exit 1; }

mapfile -t FILES < <("$ADB" shell "ls $SRC" | tr -d '\r' | grep '\.wav$' || true)
...
DAY="$DEST/$(date +%Y-%m-%d)"
mkdir -p "$DAY"
for f in "${NEW[@]}"; do "$ADB" pull -q "$SRC/$f" "$DAY/$f"; done
cd "$PRO"
git pull -q --ff-only
