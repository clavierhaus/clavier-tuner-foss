...
class NoteDetector(
    ...
    private val hann by lazy { DoubleArray(windowSize) { 0.5 - 0.5 * kotlin.math.cos(2.0 * kotlin.math.PI * it / (windowSize - 1)) } }

    /**
     * The key struck in the last [windowSize] samples of [samples] (Hann
     * window, the noise floor the median magnitude between 50 Hz and
     * 12 kHz), or null. Used only to follow the key struck: the note named
     * here is then read by phase on its own partial (docs/ENGINE.md §5).
     */
    fun detectIn(samples: FloatArray, a4Hz: Double): Int? {
        if (samples.size < windowSize) return null
        val off = samples.size - windowSize
        val re = DoubleArray(windowSize) { samples[off + it] * hann[it] }
        val im = DoubleArray(windowSize)
        at.clavierhaus.unisonmaster.dsp.Fft.transform(re, im)
        val mag = DoubleArray(windowSize / 2) { kotlin.math.sqrt(re[it] * re[it] + im[it] * im[it]) }
        ...
        return detect(mag, db(floor), a4Hz)
    }
