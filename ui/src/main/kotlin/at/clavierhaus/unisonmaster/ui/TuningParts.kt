 ...
 * labelled at every whole Hz. Each shown partial is drawn at its own offset
 * from its own target; a partial within ±0.1 Hz of its target is green.
 *
 * Readout view ([readout] true): one static light-blue bell on the line
 * stands for every target; the live partials are orange bells of one
 * common height, numbered, the [active] one on top. The frequencies are
 * read in the [ReadoutColumn] beside the graph.
 *
 * Pairs view: one blue/orange pair per partial at stepped heights, with the
 * fundamental's live frequency above the line.
 */
@Composable
fun TuningGraph(
    liveHz: Double?,
    ...
    Box(modifier) {
        ...
        Canvas(
            Modifier
                .fillMaxSize()
                .padding(top = if (readout) 24.dp else 64.dp),
        ) {
            val w = size.width
            ...
            if (readout) {
                ...
                // the active partial last, so it is never hidden
                ...
                for ((i, k) in shown.sorted().withIndex()) {
                    ...
                    if (sounding && lk != null) {
                        bellFilled(xOf(lk - tk), height, sigma, base, if (match) green else Color(Brand.ORANGE))
                    }
                    if (shown.size > 1) native.drawText("$k", xc - sigma * 1.3f, base - height + 4f, sideLabel)
                }
            }
...
/** Switches the tuning screen between the readout view and bell pairs. */
@Composable
fun TuningViewToggle(readout: Boolean, onClick: () -> Unit, modifier: Modifier = Modifier) {
    OutlinedButton(onClick = onClick, modifier = modifier) {
        Text(
            if (readout) "Pairs" else "Readout",
